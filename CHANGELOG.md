...
### P0 质量基建
- **Golden Dataset 评估体系**：`data/synthetic/` 合成数据集（12 case × 181 行 GT，覆盖库存 0/1/9/10/99/100/999/1000、销量 0/10/110/1109、多分辨率/DPI/多页变体）+ `tools/eval_accuracy.py` 评估器（行召回/字段/完整行/补货误差率 KPI，报告绝不含业务原文）+ **端到端 mock 评估**（`--use-transport` 经 `vision.set_transport` 注入，12 case 实测 100%）；3 张真实截图 GT（18 行）已标注且**严格托管在测试环境本地目录，绝不入 git**
...
### v1.6.0 发布后补充（不 bump，随入库合并）
- **发布前审查修复（5 项）**：识别 transport 桩显式失败绝不静默回退真实 API；`置信度 YELLOW→medium` 映射修正（复核弹窗不再空）；复核降级结果加「⚠降级计算」前缀；回测器自检断言数学修正；更新器源码形态回滚补警告日志（行为不变）
- **计价内置**：通义官网 15 模型官方价目内置为发布默认（含 `qwen3-vl-plus 1.0/10`、`qwen3.5-ocr 0.5/2`、`qwen-vl-max 1.6/4` 等，4 档上下文价可在设置页细分覆盖）；4 个非 token 计费模型与 4 个官方下架 slug 如实跳过；`resolve_pricing` 前缀回退全链路验证（带日期后缀/-latest/精确键均命中）

## v1.5.13（结果列开关：预警/预测/模型可关，列表与导出联动）

**用户反馈：不想看预警/预测列，导出的 Excel 也被迫带上。**

- 结果表筛选行新增「列: ☑预警 ☑预测 ☑模型」三个开关（默认全开，向后兼容）——取消勾选即从**结果表**隐藏对应列，同时**导出 Excel 同步移除**（地区/店铺/仓库/勾选列/状态/补货量等核心列不受影响）
- 配置持久化：`settings result_columns`（Config 通道原子写，重启记忆）
- 纯逻辑 `filter_result_cols()`（utils）：按**字段 key**过滤（`rmodel`/`model` 同视为模型列），供结果表与导出共用一套规则；`save/get_result_cols_cfg` 薄封装
- 导出侧 `_has_fc/_has_model/_has_warn` 同步守卫：列开关决定表头与行值同时写入与否（列宽按 kept 列对齐）
- 测试：新增 `test_result_cols.py` 14 用例（过滤规则/配置往返/接线断言）

**测试总账**：**1061 项全绿**（v1.5.12 的 1047 + 14）。

## v1.5.12（计价系统复活 + 按用途汉化）

**用户反馈：token 数值已抓到但计价瘫痪（金额恒 0）；用量明细「按用途」大量英文未汉化。**

- **官方价目抓取器**（新增 `tools/fetch_pricing_qwen.py`）：从通义官网 `qianwenai.com/models` 抓取各模型 API 价目（每百万 tokens，元），当前成功抓取 **9 个模型**（qwen3-vl-plus ¥1/¥10、qwen3.5-ocr ¥0.5/¥2、qwen-vl-ocr ¥0.3/¥0.5、qwen3-max/flash、qwen3.8-max/flash、deepseek-v4-flash、kimi-k3 等）；脚本可随时重跑刷新（价格会随官方调整）
- **价格已并入本机计价表**：`settings usage.pricing.qwen` 现含官方价目，识别调用即按实际 token 计费（实测 1M 输入 + 0.5M 输出 = ¥6.0）
- **前缀回退修"计价瘫痪"根因**：`resolve_pricing()`（usage_extractor）精确 → 快照后缀（`qwen3-vl-plus-2025-12-19`→`qwen3-vl-plus`）→ 逐段前缀回退——带日期/`-latest` 后缀的模型名此前查不到价目导致恒 0；ocr/vision 落账点已切换
- **按用途汉化**：`usage_store.display_call_site()` + 18 项中文映射（按钮/元素定位、读取官方总条数、表格识别、页面状态检测等；同义别名合规合并；未知 key 原文兜底），用量明细「按用途」树展示中文
- **测试**：新增 `test_usage_pricing.py` 16 用例（回退链/缺价零值/汉化映射/撞名校验）

**测试总账**：**1047 项全绿**（v1.5.11 的 1031 + 16）。
