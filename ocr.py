...
# 只剥"空白/串首 + 纯数字"结尾；数字前是字母/冒号等（如 'ID:9662…'、'SKU-001'）不剥
_TAIL_NUM_RE = r'(?:\A|[ \t\u3000\r\n])\d+(?:[.,，、]\d+)*[ \t\u3000\r\n]*$'
...
3. 单元格值原样抄写，不要转换数字、不要去掉单位；数字后的日期时间不抄（如"258份 08-02"只抄"258份"）。**数字类列（库存/销量）只抄第一个数值，数值后如果还有其他数字串/时间，一律不要抄**（如"102份 12345"只抄"102份"）
...
- 单元格值原样抄写，不要转换数字、不要去掉单位；数字后的日期时间不抄。**数字类列（库存/销量）只抄第一个数值，数值后如果还有其他数字串/时间，一律不要抄**（如"102份 12345"只抄"102份"）
